"""
Random Staging Docking Test Node

매 trial마다 원점 → 랜덤 스테이징(x=5.0, y∈[-0.5, +0.5]) → dock(navigate_to_staging=False) 사이클.
진입 각도를 매 trial 다르게 하여 다변화 도킹 정확도를 측정.
...
# 도킹 목표 참조 좌표 (yaw 계산용)
DOCK_REF_X = V_SHAPE_TIP[0]   # 3.006
DOCK_REF_Y = V_SHAPE_TIP[1]   # 0.008
...
class RandomStagingDockingTestNode(Node):
    """랜덤 스테이징 도킹 정확도 측정 노드."""

    def __init__(self):
        super().__init__('random_staging_docking_test_node')
        ...
        self.get_logger().info(
            f'RandomStagingDockingTestNode 초기화 완료. n_trials={self._n_trials}'
        )
    ...
    def _test_loop(self):
        ...
        for trial_idx in range(self._n_trials):
            ...
            # 2.5. 랜덤 스테이징으로 이동
            staging_y   = random.uniform(STAGING_Y_MIN, STAGING_Y_MAX)
            staging_yaw = math.atan2(DOCK_REF_Y - staging_y, DOCK_REF_X - STAGING_X)
            self.get_logger().info(
                f'스테이징 이동: x={STAGING_X}, y={staging_y:.3f}m, '
                f'yaw={math.degrees(staging_yaw):.1f}°'
            )
            staging_success = self._navigate_to_staging(staging_y, staging_yaw)
            if not staging_success:
                self.get_logger().warn('스테이징 이동 실패. 계속 진행합니다.')
    ...
    def _navigate_to_staging(self, staging_y: float, staging_yaw: float) -> bool:
        """NavigateToPose로 랜덤 스테이징 포즈로 이동."""
        goal = NavigateToPose.Goal()
        goal.pose = PoseStamped()
        goal.pose.header.frame_id = 'map'
        goal.pose.header.stamp = self.get_clock().now().to_msg()
        ...
        goal.pose.pose.position.z = 0.0
        goal.pose.pose.orientation = yaw_to_quaternion(staging_yaw)
        ...
        goal_handle = future.result()
        if goal_handle is None or not goal_handle.accepted:
            self.get_logger().warn('NavigateToPose (staging) goal 거부됨.')
            return False
    ...
    def _save_png_impl(self):
        ...
        fig.suptitle('Random Staging Docking Accuracy Test Results', fontsize=15, fontweight='bold')
    ...
    def _save_trial_png_impl(self, trial_idx: int,
        ...
        if title is None:
            title = f'Random Staging Docking Accuracy — Trial {trial_num}'
...
def main(args=None):
    ...
    node = RandomStagingDockingTestNode()
